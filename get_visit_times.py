...
def date_ymd_to_timestamp_ms(y,m,d):
    return datetime.datetime(y,m,d).timestamp() * 1000.0
...
def dist_btw_two_points(p1,p2):
    ...
    phi2 = deg2rad(p2[0])
    lba2 = deg2rad(p2[1])
    r = 6371 * 1000.0    # Earth's radius: 6371kms in meters
...
# Read the file
print("Loading 'location_history.json' ...")
with open('location_history.json') as json_file:
    main_dict = json.load(json_file)  # This can take a bit of time
...
end_ts = date_ymd_to_timestamp_ms(2020,1,25)
# end_ts = timestampMs[-1]    # Last one

# Point of interest
poi = np.array([40.382616, -111.915957])    # in degrees
...
# Converter
ts2datetime = lambda x: datetime.datetime.fromtimestamp(int(x / 1000.0)).strftime('%Y-%m-%d %H:%M:%S')
