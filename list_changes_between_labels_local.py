...
def FindChanges( project, beginLabel, endLabel ):
    command = 'p4 changes -t -s submitted "//SEABU/ProductSource/'
    ...
    while i < len( changes ):
        command = 'p4 describe -s "' + str( changes[i].mNumber ) + '"'
        lines = os.popen4( command, 't' )[1].readlines()
...
def GetLabels( project ):
    command = 'p4 label -o ' + project.mOldLabel
    lines = os.popen4( command, 't' )[1].readlines()
    ...
    if project.CompareToNow():
        ...
        command = 'p4 label -o ' + project.mNewLabel
        lines = os.popen4( command, 't' )[1].readlines()
...
def FindComponentsTxtDependencies( project ):
    ...
    command = 'p4 print -q "//SEABU/ProductSource/' + project.mCategory + '/' + project.mName + '/' + project.mOldBranch
    ...
    command = 'p4 print -q "//SEABU/ProductSource/' + project.mCategory + '/' + project.mName + '/' + project.mNewBranch
    if project.CompareToNow():
